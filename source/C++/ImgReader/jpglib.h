...
)
{
	...
	while (ndata) {	/* Process all tables in the segment */
		...
		pb = (UCHAR*)alloc_pool(jd, 16);			/* Allocate a memory block for the bit distribution table */
		if (!pb) return JDR_MEM1;			/* Err: not enough memory */
		...
		ph = (USHORT*)alloc_pool(jd, np * sizeof (WORD));/* Allocate a memory block for the code word table */
		...
		ndata -= np;
		pd = (UCHAR*)alloc_pool(jd, np);	/* Allocate a memory block for the decoded data */
...
)
{
	...
	jd->inbuf = seg = (UCHAR*)alloc_pool(jd, JD_SZBUF);		/* Allocate stream input buffer */
	...
	for (;;) {
		...
		case 0xDA:	/* SOS */
			...
			jd->mcubuf = (UCHAR*)alloc_pool(jd, (n + 2) * 64);	/* Allocate MCU working buffer */
